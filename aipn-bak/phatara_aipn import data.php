...
//17 $strSQL = "select * from aipn_running";
//17 $objQuery = mysqli_query($objConnect,$strSQL);
//17 $row_ = mysqli_fetch_all($objQuery, MYSQLI_ASSOC);
//17foreach($row_ as $row){
//17    echo 'ไฟล์ส่ง AIPN ครั้งก่อน 14354AIPN';	
//17	echo($row['last_number']);
//17}
...
//17 $last_number = $row['last_number'] + 1;
//$id = '1000'.$last_number;
//17 $id = $last_number;
//17 $my = new cipn_xlm('5800001');
//17 $my->save();
//17 $link= "http://localhost/PhataraAIPN/".$my->save_zip($id);
//$my->save_zip($id);

//17 $strSQL = "update aipn_running set last_number = " . $last_number;
//17 $objQuery = mysqli_query($objConnect,$strSQL);

// ปรับค่า จาก 99999 เป็น ค่าเริ่มต้น 10000
//17 $strSQL = "update aipn_running set last_number =if(last_number =99999,10000,last_number)";
//17 $objQuery = mysqli_query($objConnect,$strSQL);

?>
<!-- 17        <a href="<?php echo $link;?>"><?php echo $link;?></a> -->
